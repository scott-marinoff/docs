...
Note: The frequency breakdown will only count the events in between the chosen range. The event that originally satisfied the funnel definition does not count towards the frequency between steps. If you get 0 in your breakdown, it means that that event did not occur again before the next step in the funnel.
